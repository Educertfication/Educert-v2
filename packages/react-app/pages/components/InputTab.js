import {FormControl, FormLabel, Input, useDisclosure, InputGroup,InputLeftElement,InputRightElement, Checkbox, Button, Collapse } from '@chakra-ui/react'
...
const InputTab = () => {
    ...
    const { data: VerifyData, isError : isErrorVerify, isLoading: isverifyLoading, isFetched: isVerifyFetched } = useContractRead({
      address: chooseAddr,
      abi: childabi,
             ...
             <Collapse in={isOpenDrop} animateOpacity>
                ...
                {AllaccountData?.map((item, index)=>{
                  const handleAddressClick = () => {
                   ...
                   return <div key={index} className='w-[167px] my-[20px] h-[22px] satoshi font-[400] text-[16px] text-[#EEEEF06B] border-b-[2px] mx-auto cursor-pointer' onClick={handleAddressClick}>{item._name}</div>
                })}
                </div>
            </Collapse>
